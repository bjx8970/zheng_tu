/**
 * 党务线深度玩法 —— 入口文件（仅 JSX 框架）
 * 行动数据 → src/lib/deepActions/partyActions.ts
...
import {
  PARTY_ACTIONS, PARTY_CATEGORIES, PARTY_CAT_ICONS,
  type PartyCategory,
} from '@/lib/deepActions/partyActions';

export default function PartyDeep() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const [activeCategory, setActiveCategory] = useState<PartyCategory>('组织建设');
    ...
    <View style={{ flex: 1, backgroundColor: theme.pageBg }}>
      ...
      <View style={{ backgroundColor: theme.primary, paddingTop: insets.top + 8, paddingBottom: 12, paddingHorizontal: 16 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 10 }}>
          ...
          <Text style={{ color: '#fff', fontSize: 17, fontWeight: '800', flex: 1 }}>🎖️ 党务深度玩法</Text>
        ...
        <View style={{ flexDirection: 'row', gap: 6 }}>
          {PARTY_CATEGORIES.map(cat => (
            <Pressable key={cat} onPress={() => setActiveCategory(cat)}
              ...
              <Text style={{ fontSize: 13 }}>{PARTY_CAT_ICONS[cat]}</Text>
