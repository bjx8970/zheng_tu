/**
 * 纪检线深度玩法 —— 入口文件（仅 JSX 框架）
 * 行动数据 → src/lib/deepActions/disciplineActions.ts
...
import {
  DISCIPLINE_ACTIONS, DISCIPLINE_CATEGORIES, DISCIPLINE_CAT_ICONS,
  type DisciplineCategory,
} from '@/lib/deepActions/disciplineActions';

export default function DisciplineDeep() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const [activeCategory, setActiveCategory] = useState<DisciplineCategory>('巡视反腐');
    ...
    <View style={{ flex: 1, backgroundColor: theme.pageBg }}>
      ...
      <View style={{ backgroundColor: theme.primary, paddingTop: insets.top + 8, paddingBottom: 12, paddingHorizontal: 16 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 10 }}>
          ...
          <Text style={{ color: '#fff', fontSize: 17, fontWeight: '800', flex: 1 }}>⚖️ 纪检深度玩法</Text>
        ...
        <View style={{ flexDirection: 'row', gap: 6 }}>
          {DISCIPLINE_CATEGORIES.map(cat => (
            <Pressable key={cat} onPress={() => setActiveCategory(cat)}
              ...
              <Text style={{ fontSize: 13 }}>{DISCIPLINE_CAT_ICONS[cat]}</Text>
