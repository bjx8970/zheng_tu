/**
 * 行政线深度玩法 —— 入口文件（仅 JSX 框架）
 * 行动数据 → src/lib/deepActions/adminActions.ts
...
import {
  ADMIN_ACTIONS, ADMIN_CATEGORIES, ADMIN_CAT_ICONS,
  type AdminCategory,
} from '@/lib/deepActions/adminActions';

export default function AdminDeep() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const [activeCategory, setActiveCategory] = useState<AdminCategory>('城市治理');

  const {
    save, acting, savedResults, balance,
    rank, actionCost, isCool, cdLeft, handleAction,
  } = useDeepAction({ cooldownsField: 'adminDeepCooldowns', resultsField: 'adminDeepResults' });

  if (!save) return null;

  const theme      = getRankThemeWithLine(rank, '行政线');
  const catActions = ADMIN_ACTIONS.filter(a => a.category === activeCategory);
    ...
    <View style={{ flex: 1, backgroundColor: theme.pageBg }}>
      ...
      <View style={{ backgroundColor: theme.primary, paddingTop: insets.top + 8, paddingBottom: 12, paddingHorizontal: 16 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 10 }}>
          ...
          <Text style={{ color: '#fff', fontSize: 17, fontWeight: '800', flex: 1 }}>🏛️ 行政深度玩法</Text>
        ...
        <View style={{ flexDirection: 'row', gap: 6 }}>
          {ADMIN_CATEGORIES.map(cat => (
            <Pressable key={cat} onPress={() => setActiveCategory(cat)}
              ...
              <Text style={{ fontSize: 13 }}>{ADMIN_CAT_ICONS[cat]}</Text>
          ...
          return (
            <View key={action.key} style={{ backgroundColor: theme.cardBg, borderRadius: 14, borderWidth: 1, borderColor: theme.cardBorder, padding: 14 }}>
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginBottom: 10 }}>
                ...
                {(action.successOutcome.fundDelta ?? 0) > 0 && (
                  <View style={{ backgroundColor: '#ECFDF5', borderRadius: 5, paddingHorizontal: 7, paddingVertical: 3 }}>
                    <Text style={{ fontSize: 10, color: '#065F46' }}>经费+{fmtGovFund(action.successOutcome.fundDelta!)}</Text>
                  </View>
                )}
                {action.once && (
                  <View style={{ backgroundColor: '#FFF7ED', borderRadius: 5, paddingHorizontal: 7, paddingVertical: 3 }}>
                    <Text style={{ fontSize: 10, color: '#C2410C' }}>每年1次</Text>
                  </View>
                )}
                {cool && (
              ...
              {lastResult && (
                <View style={{ backgroundColor: lastResult.ok ? '#ECFDF5' : '#FEF2F2', borderRadius: 8, padding: 10, marginBottom: 10, borderWidth: 1, borderColor: lastResult.ok ? '#BBF7D0' : '#FECACA' }}>
                  <Text style={{ fontSize: 11, fontWeight: '700', color: lastResult.ok ? '#065F46' : '#B91C1C', marginBottom: 3 }}>
                    {lastResult.ok ? '✅ 上次处置' : '❌ 上次失败'} · 第{lastResult.day}天
                  </Text>
